...
class Slim {
    ...
    public static function saveFile($data, $name, $path = 'uploads/images/', $uid = true) {
        ...
        $path = $path . $name;
        $profile = $data;
        $image_upload = plugins_url('uploads/images/'.$name,__FILE__);
        
        $results = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}userprofile WHERE user_id = %d",$user_id));
        //Update user image else insert to the database.
        if($results){
             if($profile['server']['file']){
                $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}userprofile SET profile = %s, date_updated = %s, formatted_date_updated = %s WHERE user_id = %d", $image_upload,$date_updated,$string_date,$user_id ) );
            } else { 
              wp_die();
            }
        } else {
            if($profile['server']['file']){
                $wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}userprofile(user_id,profile,date_updated) values(%d,%s,%s)", $user_id,$image_upload,$date_updated ) );
            } else { 
              wp_die();
            }
        }
